...
Launchers mínimos para imprimir por `stdout` prompts canónicos guardados fuera del repo, en tu vault de trabajo.
...
Cada launcher imprime el contenido del prompt en `stdout`. Si falla, devuelve `exit != 0` y muestra un error útil en `stderr`.
...
```bash
AI_PROMPTS_VAULT_ROOT=/mnt/c/Users/jesus/Documents/vault_trabajo ./local/bin/prompt-understand-context | head
AI_PROMPTS_VAULT_ROOT=/mnt/c/Users/jesus/Documents/vault_trabajo ./local/bin/prompt-plan-safe-change | head
AI_PROMPTS_VAULT_ROOT=/mnt/c/Users/jesus/Documents/vault_trabajo ./local/bin/prompt-detect-errors | head
```
...
```bash
AI_PROMPTS_VAULT_ROOT=/tmp/no-existe ./local/bin/prompt-detect-errors
AI_PROMPTS_VAULT_ROOT=/tmp/vault-sin-prompt ./local/bin/prompt-detect-errors
```
