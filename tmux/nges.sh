...
tmux send -t $session:Local "cd /var/www/nges.local" ENTER
...
tmux split-window -v -p 35 && sleep $tiempo 
tmux send-keys -t 2 'ssh -oHostKeyAlgorithms=+ssh-dss jesus@89.17.208.189 && cd /home && tail -f /var/log/httpd/dev-ofertas_error_log' C-m && sleep $tiempo
