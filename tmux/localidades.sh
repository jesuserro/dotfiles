...
tmux send-keys -t 2 'ssh -oHostKeyAlgorithms=+ssh-dss subv@ms.cdrst.com' C-m && sleep $tiempo
tmux split-window -v -p 35 && sleep $tiempo 
tmux send-keys -t 2 'ssh -oHostKeyAlgorithms=+ssh-dss subv@ms.cdrst.com && tail -f /var/log/httpd/dev-ofertas_error_log' C-m && sleep $tiempo
